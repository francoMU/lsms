...
void getCoreStates(LSMSSystemParameters &lsms, AtomData &atom) {
    ...
    */
    ...
    // New core density
    std::vector<Real> corden_new(atom.corden.l_dim(), 0);
    ...
    std::vector<Real> Q(N, 0);
    std::vector<Real> V(N, 0);
    std::vector<Real> Rp(N, 0);

    for (int i = 0; i < atom.vr.l_dim(); i++) {
        V[i] = atom.vr(i, 0) / (atom.r_mesh[i] * 2.0);
    ...
    for (int is = 0; is < lsms.n_spin_pola; is++) {
        ...
        for (int ic = 0; ic < atom.numc; ic++) {
            ...
            double Emax_init{0};
            double Emin_init{-500};
            bool perturb{true};
            int relat{0};
            double Ein = -atom.ztotss * atom.ztotss / (atom.nc(ic, is) * atom.nc(ic, is) * 7);
