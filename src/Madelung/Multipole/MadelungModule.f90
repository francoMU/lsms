...
module MadelungModule
   ...
   type (c_ptr), bind(C, name='DL_factor_ptr') :: DL_factor_ptr
   !
   real (kind=RealKind), target :: a0, alat
   type (c_ptr), bind(C, name='alat_ptr') :: alat_ptr
   ...
   type (c_ptr), bind(C, name='DL_matrix_ptr') :: DL_matrix_ptr
...
contains
   ...
   subroutine c_getMadelungMatrix(local_atom_index, madelung_matrix, madelung_matrix_size) &
      ...
      madelung_matrix=c_loc(madmat(1, local_atom_index))
      madelung_matrix_size=size(madmat(1:GlobalNumAtoms, local_atom_index))
   ...
   !
   !  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine c_getDLMatrix(local_atom_index, matrix, &
         atom_size, k_size, afac) &
         bind(C, name="getDLMatrix")
      use iso_c_binding, only : c_int, c_double, c_ptr, c_loc

      integer (kind=c_int), value, intent(in) :: local_atom_index !< Global number of atoms
      type(c_ptr), intent(out) :: matrix
      integer (kind=c_int), intent(out) :: atom_size
      integer (kind=c_int), intent(out) :: k_size
      real (kind=c_double), intent(out) :: afac

      matrix=c_loc(DL_matrix(1:GlobalNumAtoms, 1:kmax_mad, local_atom_index))
      atom_size=size(DL_matrix(1:GlobalNumAtoms, 1:kmax_mad, local_atom_index), 1)
      k_size=size(DL_matrix(1:GlobalNumAtoms, 1:kmax_mad, local_atom_index), 2)
      afac=alat

   end subroutine c_getDLMatrix
   ...
   !
   !  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine c_getDLFactor(j_index, matrix, &
         matrix_size) &
         bind(C, name="getDLFactor")
      use iso_c_binding, only : c_int, c_double, c_ptr, c_loc

      integer (kind=c_int), value, intent(in) :: j_index
      type(c_ptr), intent(out) :: matrix
      integer (kind=c_int), intent(out) :: matrix_size

      matrix=c_loc(DL_factor(1:kmax_mad, j_index))
      matrix_size=size(DL_factor(1:kmax_mad, j_index))

   end subroutine c_getDLFactor
   !  ===================================================================
   !
   !  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
