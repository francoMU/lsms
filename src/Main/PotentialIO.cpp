    ...
    for (int i = 0; i < crystal.num_types; i++) {
      ...
      if (lsms.pot_in_type == 0) // LSMS_1 style HDF5
      {
        ...
        if (crystal.types[i].node == comm.rank) {
          ...
          communicateSingleAtomData(comm, comm.rank, crystal.types[i].node, crystal.types[i].local_id,
                                    pot_data);
        }
        H5Gclose(fid_1);
...
void readjustPotential(LSMSCommunication &comm, LSMSSystemParameters &lsms, CrystalParameters &crystal,
                       LocalTypeInfo &local) {
...
void initialAtomSetup(LSMSCommunication &comm, LSMSSystemParameters &lsms, CrystalParameters &crystal,
                      LocalTypeInfo &local) {
  for (int i = 0; i < local.num_local; i++) {
    // local.atom[i].generateRadialMesh();
    local.atom[i].ztotss = (Real) crystal.types[local.global_id[i]].Z;
