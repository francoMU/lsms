...
void printCompressedCrystalParameters(FILE *f, CrystalParameters &crystal, int rank) {
    ...
    for (int i = 0; i < crystal.num_atoms; i++) {
        fprintf(f, "%5d %s [%d.%d]: %20.14f %20.14f %20.14f %lf %lf %lf\n",
                i + 1,
                crystal.types[i].name,
                crystal.types[i].node,
                crystal.types[i].local_id,
                crystal.position(0, i),
                crystal.position(1, i),
...
void printCrystalParameters(FILE *f, CrystalParameters &crystal) {
    ...
    for (int i = 0; i < crystal.num_atoms; i++) {
        fprintf(f, "Atom %7d : %lf %lf %lf\n", i + 1, crystal.position(0, i), crystal.position(1, i),
                crystal.position(2, i));
        fprintf(f, "             : evec = %lf %lf %lf\n", crystal.evecs(0, i), crystal.evecs(1, i),
                crystal.evecs(2, i));
        fprintf(f, "             : type = %d\n", crystal.type[i] + 1);
    }
    for (int i = 0; i < crystal.num_types; i++) {
        fprintf(f, "Type %7d : %s [%d.%d]\n", i + 1, crystal.types[i].name, crystal.types[i].node,
                crystal.types[i].local_id);
...
void printLIZInfo(FILE *f, AtomData &atom) {
    ...
    for (int i = 0; i < atom.numLIZ; i++)
        fprintf(f, "%4d %12.6lf %4d %4d %12.6lf %12.6lf %12.6lf\n",
                i,
                atom.LIZDist[i],
                atom.LIZGlobalIdx[i],
                atom.LIZStoreIdx[i],
