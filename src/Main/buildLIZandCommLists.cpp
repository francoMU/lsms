...
bool nodeIsInList(int val, std::vector<LIZInfoType> &list, int len, CrystalParameters &crystal, int &ret) {
    for (int i = ret; i < len; i++)
        if (crystal.types[crystal.type[list[i].idx]].node == val) {
            ret = i;
            return true;
        }
    return false;
...
// the first num_local entries in tmatStore contain the local tmats
    local.tmatStoreGlobalIdx.clear();
...
// i.e find min distance between atom and local atoms and compare with max liz radius
            ...
            ret = 0;
            while (nodeIsInList(comm.rank, tempLIZ, tempNumLIZ, crystal,
                                ret)) // the remote node needs the tmat from our node
            {
...
// for FROM remove all duplicate atom types
    std::vector<NodeIdxInfo>::iterator it = std::unique(fromList.begin(), fromList.begin() + fromListN,
                                                        globalEq_NodeIndexInfo);
...
// count the nodes in toList and fromList
    ...
    if (!fromList.empty()) {
        ...
        for (int i = 0; i < fromList.size(); i++) {
            if (lsms.global.iprint > 0)
                printf("fromList[%d].node=%d .localIdx=%d .globalIdx=%d\n", i, fromList[i].node,
                       fromList[i].localIdx, fromList[i].globalIdx);
            if (h != fromList[i].node) {
                h = fromList[i].node;
                numFromNodes++;
                fromCounts[numFromNodes - 1] = 1;
            } else fromCounts[numFromNodes - 1]++;
        }
    }
