...
#define PRINT_TAUOO
      ...
      for (int j = 0; j < local.num_local * lsms.n_spin_pola / lsms.n_spin_cant; j++) {

        int global = local.global_id[j];

        std::string name = std::to_string(global) + "_" + std::to_string(iie)
            + ".txt";
        std::ofstream tau_file(name.c_str());
        if (tau_file.is_open()) {

          for (int i = 0; i < maxkkrsz_ns * maxkkrsz_ns; i++) {
            tau_file << std::setw(32) << std::setprecision(18) <<
              std::scientific << tau00_l(i, j).real() << " ";
            tau_file << std::setw(32) << std::setprecision(18) <<
              std::scientific << tau00_l(i, j).imag() << std::endl;
          }
