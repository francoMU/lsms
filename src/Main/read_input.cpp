      ...
      for (int iy = 0; iy < ny; iy++)
         for (int iz = 0; iz < nz; iz++) {
            for (int ib = 0; ib < numBasis; ib++) {
               crystal.position(0, i) = basis(0, ib) + ix * crystal.bravais(0, 0) + iy * crystal.bravais(0, 1) +
                                        iz * crystal.bravais(0, 2);
               crystal.position(1, i) = basis(1, ib) + ix * crystal.bravais(1, 0) + iy * crystal.bravais(1, 1) +
                                        iz * crystal.bravais(1, 2);
               crystal.position(2, i) = basis(2, ib) + ix * crystal.bravais(2, 0) + iy * crystal.bravais(2, 1) +
...
// Read atom positions and evecs
   ...
   for (int i = 0; i < crystal.num_atoms; i++) {
      ...
      if (!luaGetIntegerFieldFromStack(L, "type", &t) || (t - 1) == i) {
         ...
         crystal.types[crystal.num_types].pot_in_idx = -1;luaGetIntegerFieldFromStack(L, "pot_in_idx", &crystal.types[crystal.num_types].pot_in_idx);
         ...
         luaGetIntegerFieldFromStack(L, "Zv", &crystal.types[crystal.num_types].Zv);crystal.types[crystal.num_types].forceZeroMoment = 0;
         ...
         for (int j = 0; j < 4; j++)
            luaGetRealPositionFromStack(L, j + 1, &crystal.types[crystal.num_types].rsteps[j]);
         lua_pop(L, 1);
   ...
   /*
    * Set repeat for forces calculation
    */
   luaGetInteger(L, "xForceRepeat", &lsms.forceParams.i_x);
   luaGetInteger(L, "yForceRepeat", &lsms.forceParams.i_y);
   luaGetInteger(L, "zForceRepeat", &lsms.forceParams.i_z);
   luaGetReal(L, "radiusForce", &lsms.forceParams.radius);


   /*
    *
    */
   luaGetInteger(L, "max_iterations", &lsms.relaxParams.max_iterations);
   luaGetReal(L, "tolerance", &lsms.relaxParams.tolerance);
   luaGetReal(L, "initial_sigma", &lsms.relaxParams.initial_sigma);
   luaGetBoolean(L, "write_to_file", &lsms.relaxParams.write_to_file);
