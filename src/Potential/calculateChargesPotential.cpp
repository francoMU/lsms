        ...
        switch (chargeSwitch) {
            case 1: {
                ...
                if (local.atom[i].rhotot(local.atom[i].jmt, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhotot(local.atom[i].jmt, lsms.n_spin_pola - 1) = local.atom[i].rhotot(
                ...
                getqm_mt_(&lsms.n_spin_pola, &local.atom[i].jmt, &local.atom[i].rInscribed, rTemp,
                          &local.atom[i].rhotot(0, 0), &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa,
            ...
            default: {
                ...
                if (local.atom[i].rhoNew(local.atom[i].jmt, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhoNew(local.atom[i].jmt, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(
                            local.atom[i].jmt - 1, lsms.n_spin_pola - 1);
                ...
                getqm_mt_(&lsms.n_spin_pola, &local.atom[i].jmt, &local.atom[i].rInscribed, rTemp,
                          &local.atom[i].rhoNew(0, 0), &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa,
                          &local.atom[i].qtotmt, &local.atom[i].mtotmt, &rSphere, &lsms.global.iprint);
            }
        }
        ...
        switch (chargeSwitch) {
            case 1: {
                ...
                if (local.atom[i].rhotot(jmt, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhotot(jmt, lsms.n_spin_pola - 1) = local.atom[i].rhotot(jmt - 1,
                                                                                           lsms.n_spin_pola - 1);
                if (local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1) = local.atom[i].rhotot(jmt - 1,
                                                                                               lsms.n_spin_pola - 1);
                ...
                if (lsms.global.iprint > 0) {
                    ...
                    printf("Spin = 2 rhotot[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                           ...
                           local.atom[i].rhotot(jmt, lsms.n_spin_pola - 1),
                           local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1));
                }

                getqm_mt_(&lsms.n_spin_pola, &jmt, &local.atom[i].rInscribed, rTemp, &local.atom[i].rhotot(0, 0),
                          &lsms.global.iprpts, &rhoTemp(0, 0, i), &lsms.mtasa, &local.atom[i].qtotmt,
                          &local.atom[i].mtotmt, &rSphere, &lsms.global.iprint);
            ...
            default: {
                ...
                if (local.atom[i].rhoNew(jmt, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhoNew(jmt, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(jmt - 1,
                                                                                           lsms.n_spin_pola - 1);
                if (local.atom[i].rhoNew(local.atom[i].jmt + 1, lsms.n_spin_pola - 1) == 0.0)
                    local.atom[i].rhoNew(jmt + 1, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(jmt - 1,
                                                                                               lsms.n_spin_pola - 1);
                ...
                if (lsms.global.iprint > 0) {
                    ...
                    printf("Spin = 2 rhoNew[jmt-1], [jmt], [jmt+1] = %20.16f %20.16f %20.16f\n",
                           ...
                           local.atom[i].rhoNew(jmt, lsms.n_spin_pola - 1),
                           local.atom[i].rhoNew(jmt + 1, lsms.n_spin_pola - 1));
                }
        ...
        case 1:             // ASA (not implemented)
            ...
            for (int i = 0; i < local.num_local; i++) {
                ...
                if (chargeSwitch == 1) {
                    ...
                    if (local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1) == 0.0)
                        local.atom[i].rhotot(jmt + 1, lsms.n_spin_pola - 1) = local.atom[i].rhotot(jmt,
                                                                                                   lsms.n_spin_pola -
                                                                                                   1);

                    if (local.atom[i].rhotot(jmt + 2, lsms.n_spin_pola - 1) == 0.0)
                        local.atom[i].rhotot(jmt + 2, lsms.n_spin_pola - 1) = local.atom[i].rhotot(jmt,
                                                                                                   lsms.n_spin_pola -
                                                                                                   1);

                    getqm_mt_(&lsms.n_spin_pola, &jmt, &local.atom[i].rInscribed, &rTemp[0],
                              &local.atom[i].rhotot(0, 0),
                    ...
                    if (local.atom[i].rhoNew(jmt + 1, lsms.n_spin_pola - 1) == 0.0)
                        local.atom[i].rhoNew(jmt + 1, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(jmt,
                                                                                                   lsms.n_spin_pola -
                                                                                                   1);

                    if (local.atom[i].rhoNew(jmt + 2, lsms.n_spin_pola - 1) == 0.0)
                        local.atom[i].rhoNew(jmt + 2, lsms.n_spin_pola - 1) = local.atom[i].rhoNew(jmt,
                                                                                                   lsms.n_spin_pola -
                                                                                                   1);
            ...
            if (i == 0) {
                ...
                for (int ir = 0; ir < local.atom[0].r_mesh.size(); ir++)
                    fprintf(of, "%d %lg   %lg %lg   %lg %lg  %lg %lg\n", ir, local.atom[i].r_mesh[ir],
                            ...
                            local.atom[i].exchangeCorrelationPotential(ir, 0),
                            local.atom[i].exchangeCorrelationPotential(ir, 1),
                            local.atom[i].exchangeCorrelationEnergy(ir, 0),
                            local.atom[i].exchangeCorrelationEnergy(ir, 0));
                fclose(of);
