...
void calculateTotalEnergy(LSMSCommunication &comm, LSMSSystemParameters &lsms, LocalTypeInfo &local,
...
*/
      ...
      electroStaticEnergy += local.atom[i].energyStruct.coloumb();
    }
...
*/
  ...
  globalSum(comm, electroStaticEnergy);
  ...
  for (int i = 0; i < local.num_local; i++) {
    ...
    local.atom[i].interstitialEnergy = 0.0;
    local.atom[i].siteLocalEnergy = local.atom[i].localEnergy;

    for (int is = 0; is < lsms.n_spin_pola; is++) {

      local.atom[i].interstitialEnergy += emad[is] / Real(lsms.num_atoms);
  ...
  lsms.electroStaticEnergy = electroStaticEnergy + lsms.u0;
